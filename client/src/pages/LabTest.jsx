...
const LabTestMedipedia = () => {
  ...
  return (
    <>
      ...
      >
        <h1
          className={`text-2xl md:text-3xl mb-6 mt-4 font-semibold ${dark === 'dark' ? 'text-yellow-400' : 'text-gray-700'}`}
        >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {tests.slice(0, currentItem).map((test, index) => (
            <div
              key={test.id}
              className={`p-3 rounded-lg shadow-lg transition-opacity duration-1000 flex flex-col h-full ${dark === 'dark' ? 'bg-gray-800' : 'bg-white'}
            ...
            >
              ...
              <div className="content flex flex-col flex-1">
                <h3
                  className={`text-base font-semibold mb-1 ${dark === 'dark' ? 'text-yellow-400' : 'text-gray-700'}`}
                >
                  {test.title}
                </h3>
                <p
                  className={`text-xs leading-relaxed py-2 flex-1 ${dark === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}
                >
                  {test.description}
                </p>
                ...
                <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-700">
                  ...
                  <span
                    className={`text-xs ${dark === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}
                  >
                    <Rating
                      name="read-only"
                      value={test.rating}
                  ...
                  <span
                    className={`text-xs ${dark === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}
                  >
                    <i className="fas fa-user text-blue-600 pr-1"></i> by
                    MedSync
                  </span>
                </div>
                <button
                  onClick={() => navigate('/hospitals-around')}
                  className={`inline-block px-4 py-1.5 mt-3 border text-sm ${dark === 'dark' ? 'border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-gray-900' : 'border-gray-700 text-gray-700 hover:bg-blue-600 hover:text-white'} transition-all duration-300 rounded-[5px]`}
                >
                  Read more
                </button>
              </div>
        ...
        {currentItem < tests.length && (
          <button
            className={`inline-block mt-6 px-6 py-3 border text-sm ${dark === 'dark' ? 'border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-gray-900' : 'border-gray-700 text-gray-700  hover:bg-blue-600 hover:text-white'} cursor-pointer rounded-[5px] transition-all duration-300`}
            onClick={loadMore}
          >
            Load more
          </button>
