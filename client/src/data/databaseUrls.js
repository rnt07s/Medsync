...
if (window.location.hostname === 'localhost') {
  // If localhost, use the local backend (backend runs on 8081 by default)
  // Allow override via environment variable `REACT_APP_API_URL`.
...
export const databaseUrls = {
  auth: {
    ...
    editProfile: adjustUrl(`${BASE_URL}/auth/profile/edit`),
    addDoctor: adjustUrl(`${BASE_URL}/auth/profile/adddoctor`),
    getAllDoctors: adjustUrl(`${BASE_URL}/auth/doctors`),
  },
  hospitals: {
    ...
    updateAppointments: adjustUrl(`${BASE_URL}/hospitalapi/_id/appointments`),
  },
  patient: {
    appointments: adjustUrl(`${BASE_URL}/patientapi/appointments`),
  },
  appointments: {
    register: adjustUrl(`${BASE_URL}/appointments/register`),
    availableSlots: adjustUrl(`${BASE_URL}/appointments/available-slots`),
  },
};
