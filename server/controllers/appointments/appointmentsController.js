...
const bookAppointmentByHospitalID = async (req, res) => {
  try {
    console.log('--- bookAppointmentByHospitalID called ---');
    console.log('Headers x-auth-token:', req.header('x-auth-token'));
    console.log('Request body before JWT fallback:', JSON.stringify(req.body));
    // If client didn't send userId, but provided a JWT in x-auth-token, use it
    const token = req.header('x-auth-token');
    if (!req.body.userId && token) {
      try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET);
        // decoded.user.id is how we sign tokens elsewhere
        if (decoded && decoded.user && decoded.user.id) {
          req.body.userId = decoded.user.id;
          console.log('Extracted userId from token:', req.body.userId);
        }
      } catch (e) {
        console.warn('Invalid JWT provided for booking, continuing without token fallback');
      }
    }

    console.log('Request body before validation:', JSON.stringify(req.body));
    const parsedData = appointmentSchema.parse(req.body);
    const hospitalId = req.params.id;

    console.log('Parsed booking data:', JSON.stringify(parsedData));

    // If hospitalId isn't a valid ObjectId (e.g. frontend sample uses '1'),
    // attempt a best-effort mapping: pick the first hospital in DB and use it as target.
    // This allows local/sample frontend data (with numeric ids) to still book when DB has hospitals.
    let hospital;
    let resolvedHospitalId = null;
    if (!mongoose.Types.ObjectId.isValid(hospitalId)) {
      console.warn('Non-ObjectId hospitalId provided:', hospitalId, '- falling back to first DB hospital (if any)');
      hospital = await Hospital.findOne();
      if (!hospital) {
        return res.status(404).json({ message: 'Hospital not found' });
      }
      // overwrite hospitalId with real id for user record
      console.log(`Mapped provided id ${hospitalId} -> using hospital _id ${hospital._id}`);
      resolvedHospitalId = hospital._id;
    } else {
      hospital = await Hospital.findById(hospitalId);
      resolvedHospitalId = hospital ? hospital._id : null;
    }
    // Resolve user only if userId provided (support guest bookings)
    let user = null;
    ...
    // Add the appointment to the user's record as well (only if user exists)
    if (user) {
    ...
    console.error('bookAppointmentByHospitalID unexpected error:', error && error.stack ? error.stack : error);
    // Return a concise message to client and log stack for debugging
    res.status(500).json({ message: 'Error booking appointment', error: (error && error.message) || 'Internal Server Error' });
  }
};
